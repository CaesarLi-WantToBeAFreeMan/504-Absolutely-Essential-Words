...
public class ViewLearnedWordsWindow extends JFrame{
	private static final long serialVersionUID = 1L;
	private User user = new User();
	private DatabaseConnector dbc = new DatabaseConnector();
	
	ViewLearnedWordsWindow(){
		this.setSize(500, 500);
		this.getContentPane().setBackground(Color.BLACK);
		this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		this.setTitle("View Learned Words Status");
		...
		for(int i = 0; i < 42; i++) {
			...
			learnStatusContainer.setLayout(new GridLayout(3, 4, 10, 10));
			...
			for(int j = 0; j < 12; j++) {
				...
				word.setForeground(user.getLessonWordLearnStatus(i, j) ? Color.green : Color.RED);
				learnStatusContainer.add(word);
			}
		}
